...
IniFile := StrReplace(A_ScriptDir "\" A_ScriptName, ".ahk", "") ".ini"
...
if !FileExist(IniFile)
{
    ...
    {
        ; 检查exe文件是否在脚本目录中
        If (exePath != A_ScriptDir "\" SubStr(exePath, (InStr(exePath, "\", 0, -1) + 1)<1 ? (InStr(exePath, "\", 0, -1) + 1)-1 : (InStr(exePath, "\", 0, -1) + 1)))
            IniWrite(exePath, IniFile, "Settings", "target_name")
        Else
            IniWrite(SubStr(exePath, (InStr(exePath, "\", 0, -1) + 1)<1 ? (InStr(exePath, "\", 0, -1) + 1)-1 : (InStr(exePath, "\", 0, -1) + 1)), IniFile, "Settings", "target_name")
        ; 写入OtherParams1
