...
ShowHelp:
{
    ...
    MsgBox, 64, 帮助, 
    (
    程序使用说明, 本程序用于精确调整目标窗口显示区的尺寸。

    按 Ctrl+Shift+`` (Esc下面的键) 热键启动窗口调整操作。

    运行流程如下：
    　1. 自动获取当前活动窗口，并显示窗口信息供用户确认;
    　2. 依次输入目标显示区的宽度和高度;
    　3. 动态计算窗口边框与标题栏占用的像素，并调整整体窗口尺寸;
    　4. 调整后会显示验证信息;
    　5. 根据提示选择是否结束程序。

    你可通过托盘菜单中的“退出程序”来结束本程序。

                By Luminous feat.  Microsoft Copilot Think Deeper
