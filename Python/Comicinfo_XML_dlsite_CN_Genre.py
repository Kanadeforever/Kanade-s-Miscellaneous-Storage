...
def fetch_genre_tags(url: str, log_path=None) -> str:
    ...
    try:
        ...
        # -- 修改 URL，追加 locale=zh_CN 参数以获取中文标签 --
        parsed = urllib.parse.urlparse(original_url)
        query_params = urllib.parse.parse_qs(parsed.query, keep_blank_values=True)
        # 强制设置为中文
        query_params['locale'] = ['zh_CN']
        new_query = urllib.parse.urlencode(query_params, doseq=True)
        localized_url = urllib.parse.urlunparse((
        ...
        if not div_match:
            write_log(f"[标签] 未找到 main_genre div: {original_url}", log_path)
            return ""
        ...
        result = ", ".join(tags)
        if result:
            write_log(f"[标签] ✓ 获取到 {len(tags)} 个标签（中文）: {result}", log_path)
        else:
